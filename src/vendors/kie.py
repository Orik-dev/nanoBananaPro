...
class KieClient:
    """
    ✅ ПРОСТОЕ РЕШЕНИЕ: Pro работает как Standard, только параметры другие
    """
    
    # ✅ Валидные значения из документации nano-banana-pro
    VALID_ASPECT_RATIOS = {
        "1:1", "2:3", "3:2", "3:4", "4:3", 
    ...
    async def create_task(
        ...
        *,
        ...
        """
        ✅ ПРОСТОЕ РЕШЕНИЕ: Одинаковая логика, разные параметры
        """
        ...
        # ✅ Pro поддерживает до 5000 символов
        max_len = 5000 if user_model == "pro" else 2000
        ...
        # 🔥 КЛЮЧЕВОЕ ИЗМЕНЕНИЕ: Разные параметры, но ОДИНАКОВАЯ структура
        if user_model == "pro":
            # 🎯 PRO: используем image_input, aspect_ratio, resolution
            
            # ✅ Валидация aspect_ratio
            aspect_ratio = image_size or "1:1"
            if aspect_ratio not in self.VALID_ASPECT_RATIOS:
                log.warning(_j(
                    "kie.create.invalid_aspect_ratio",
                    cid=cid,
                    provided=aspect_ratio,
                    using="1:1"
                ))
                aspect_ratio = "1:1"
            ...
            payload: Dict[str, Any] = {
                ...
                "input": {
                    ...
                    "aspect_ratio": aspect_ratio,  # ✅ ВМЕСТО image_size
                    "resolution": "4K",             # ✅ НОВЫЙ ПАРАМЕТР
                }
            }
            
            # 🔥 КРИТИЧНО: image_input ВМЕСТО image_urls
            if has_images:
                payload["input"]["image_input"] = image_urls[:10]  # ✅ До 10 изображений
            else:
                payload["input"]["image_input"] = []  # ✅ Пустой массив
                
        else:
            # 📦 STANDARD: старая логика БЕЗ ИЗМЕНЕНИЙ
            payload: Dict[str, Any] = {
                "model": model,
                "input": {
                    "prompt": prompt,
        ...
        log.info(_j(
            ...
            resolution=payload["input"].get("resolution", "default"),
            aspect_ratio=payload["input"].get("aspect_ratio", payload["input"].get("image_size")),
            image_param="image_input" if user_model == "pro" else "image_urls",
    ...
    async def get_status(
        ...
        *,
        ...
        """Получение статуса - БЕЗ ИЗМЕНЕНИЙ"""
        max_attempts = 3
        delay = 2.0
    ...
    async def wait_until_done(
        ...
        *,
        ...
        """Ожидание завершения - БЕЗ ИЗМЕНЕНИЙ"""
